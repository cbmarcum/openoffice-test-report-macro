...
/*
 * BulkImportResultXmlMacro.groovy
 ...
 * Expects results from 3 runs each of BVT and FVT tests.
...
def openXmlDialog = new JFileChooser(
        dialogTitle: "Choose a directory that contains the output directories",
        fileSelectionMode: JFileChooser.DIRECTORIES_ONLY,
        multiSelectionEnabled: false)

if (openXmlDialog.showOpenDialog() == JFileChooser.APPROVE_OPTION) {
    File topDir = openXmlDialog.getSelectedFile()
    String topDirStr = topDir.getPath()
    println(topDirStr)

    List<String> testRuns = ["bvt-1", "bvt-2", "bvt-3", "fvt-1", "fvt-2", "fvt-3"]
    Map<String,XSpreadsheet> sheetMap = [:]

    // load up null spreadsheets
    testRuns.each { run ->
        XSpreadsheet ss = null
        sheetMap."${run}" = ss
    }
    ...
    XSpreadsheetDocument xSpreadsheetDocument = xModel.guno(XSpreadsheetDocument.class)
    ...
    try {
        ...
        System.out.println("Getting spreadsheets")
        XSpreadsheets xSheets = xSpreadsheetDocument.sheets

        int i = 0
        testRuns.each { run ->
            // insert the sheets
            xSheets.insertNewByName(run, (short) i++)
            // put the sheet in the map
            sheetMap."${run}" = xSpreadsheetDocument.getSheetByName(run)
        }
    ...
    XPropertySet colPs = xSpreadsheetDocument.getCellStylePropertySet("ColHeading")
    ...
    XPropertySet rowPs = xSpreadsheetDocument.getCellStylePropertySet("RowHeading")
    rowPs["HoriJustify"] = CellHoriJustify.LEFT
    rowPs["CellBackColor"] = LIGHTER_GRAY
    rowPs["CharWeight"] = com.sun.star.awt.FontWeight.BOLD

    XPropertySet oddRowPs = xSpreadsheetDocument.getCellStylePropertySet("OddRow")
    ...
    XPropertySet evenRowPs = xSpreadsheetDocument.getCellStylePropertySet("EvenRow")
    ...
    XPropertySet redBgPs = xSpreadsheetDocument.getCellStylePropertySet("RedBg")
    redBgPs["IsCellBackgroundTransparent"] = false
    redBgPs["CellBackColor"] = CRIMSON
    redBgPs["CharColor"] = WHITE


    // from here we iterate each test run
    testRuns.each { String run ->

        XSpreadsheet sht = sheetMap."${run}"
        xSpreadsheetView.setActiveSheet(sht)
        xFreeze = xController.guno(XViewFreezable.class)

        xmlFile = new File(topDirStr + "/output." + run + "/result.xml")
